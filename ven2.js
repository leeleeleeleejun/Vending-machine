...
changeButton.addEventListener('click', () => {
  ...
  if (
    ...
    myMoneyValue.textContent -= changeCoin.value * 900;
    myMoneyValue.textContent = addComma(myMoneyValue.textContent);
    changeCoin.value = '';
...
const inputEvent = (inputTarget, machineBalance) => {
  ...
  inputTarget.textContent = addComma(inputTarget.textContent);
  machineBalance.textContent = addComma(machineBalance.textContent);
...
const returnBalance = (returnTarget, my) => {
  if (returnTarget.textContent.includes(',')) {
    returnTarget.textContent = returnTarget.textContent.replace(',', '');
  }
  if (my.textContent.includes(',')) {
    my.textContent = my.textContent.replace(',', '');
  }
  my.textContent = addComma(
    Number(returnTarget.textContent) + Number(my.textContent)
  );
...
const itemEvent = (() => {
    ...
    pushItem(eCurrentTarget) {
      ...
      cancelButtonElement.addEventListener('click', () => {
        ...
        if (this.useCoin >= price / 1000) {
          ...
          machineBalanceMoney.textContent = addComma(
            machineBalanceMoney.textContent
          );
          this.useCoin = 0;
        }
      ...
      liElement.addEventListener('click', (e) => {
        ...
        if (e.shiftKey) {
          ...
          cancelActiveShiftKey.push(e.currentTarget);
        }
      });
      liElement.addEventListener('dragstart', (e) => {
        if (cancelActiveShiftKey.includes(liElement)) {
          checkShiftKey = true;
        }
      ...
      menuList.addEventListener('drop', (e) => {
        if (checkShiftKey) {
          [...getItemList.children].forEach((shiftKeyItem) => {
            if (cancelActiveShiftKey.includes(shiftKeyItem)) {
              for (
                let i = 0;
  ...
  const pushEventWrap = (e, id, price) => {
    ...
    machineBalanceMoney.textContent = addComma(machineBalanceMoney.textContent);
    basketTotalFunction();
  };
  ...
  getButton.addEventListener('click', () => {
    ...
    totalMoney.textContent = addComma(totalMoney.textContent);
    if (Number(basketTotalSale.textContent.replace(',', '')) > 0) {
  ...
  const basketTotalFunction = () => {
    ...
    if (Number(basketTotal.textContent) >= 10000) {
      ...
      basketTotalSale.textContent = addComma(basketTotalSale.textContent);
    } else {
      basketTotalSale.textContent = 0;
    }
    basketTotal.textContent = addComma(basketTotal.textContent);
  };
